...
- **工具切换**：侧栏新增「画笔 / 选框」切换按钮，分别对应 `B` / `M` 快捷键。
- **矩形选区**：在选框模式下点击或拖拽画布以创建选区，可通过按钮或快捷键 `F` 填充、`Esc` 取消。
- **吸管与缩放**：按住 `Alt`（桌面端）或触摸长按可吸取颜色；触摸双指或滚轮调整缩放，双击触摸可快速缩放。
- **分享链接**：使用「复制分享链接」按钮生成当前画布的 URL，并可在任意页面通过「应用当前链接」载入。
