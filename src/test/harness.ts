...
const harness: HarnessState = (globalThis.__pixelHarness ||= {
  suiteStack: [],
...
async function runHooks(contexts: ReadonlyArray<SuiteContext>) {
...
function formatError(error: unknown, depth: number): string {
...
export const it = (name: string, fn: () => unknown | Promise<unknown>) => {
...
class Expectation<T> {
  ...
  toContain(expected: unknown) {
    const container: any = this.actual as any
  ...
  toHaveBeenCalledTimes(times: number) {
    const calls = (this.actual as any)?.mock?.calls
  ...
  toMatchObject(expected: Record<string, unknown>) {
    ...
    for (const [key, value] of Object.entries(expected)) {
      if (!isDeepStrictEqual((this.actual as any)[key], value)) {
        throw new Error(`Expected property ${key} to match ${formatValue(value)}, received ${formatValue((this.actual as any)[key])}`)
      }
    }
  }
...
export const expect = <T>(value: T) => new Expectation(value)

const createSpy = (target: Record<string, any>, key: string) => {
  const original = target[key]
  if (typeof original !== 'function') {
    throw new Error(`Cannot spy on property ${key}`)
  }
  const calls: any[][] = []
...
export const vi = {
  spyOn(target: Record<string, any>, key: string) {
